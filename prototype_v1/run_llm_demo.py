...
def main() -> None:
    ...
    json_path = RESULTS_DIR / "llm_demo_output.json"
    md_path = RESULTS_DIR / "llm_demo_report.md"
    ...
    with log_path.open("a", encoding="utf-8") as file:
        file.write(
            f"[{now}] 完成一次 llm demo 运行，输出：{json_path.name}, {md_path.name}，"
            f"narrator={sandbox.narrator.describe()}\n"
        )

    print("LLM demo finished.")
    print(f"JSON result: {json_path}")
    print(f"Markdown report: {md_path}")
    print(f"Narrator: {sandbox.narrator.describe()}")
